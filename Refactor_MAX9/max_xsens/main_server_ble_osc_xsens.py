...
def get_local_ip_legacy():
    """Get local IP address of machine (legacy helper, not used for OSC client)"""
    try:
...
def ask_user_for_mode():
    """Prompt user to select which payload/data mode to use."""
    print("\nSelect data mode:")
    items = list(PAYLOAD_MODES.items())
    for i, (name, code) in enumerate(items, start=1):
        rate = MODE_TO_RATE.get(code, "?")
        print(f"  {i}. {name}  (code={code}, rate={rate} Hz)")

    while True:
        choice = input("Enter option number (e.g. 1): ").strip()
        if not choice.isdigit():
            print("Please enter a number.")
            continue
        idx = int(choice)
        if 1 <= idx <= len(items):
            name, code = items[idx - 1]
            print(f"→ Selected: {name} (code={code})")
            return code
        else:
...
osc_port = OSC_PORT  # use configured OSC_PORT (5555)
...
async def main():
    ...
    selected_mode = ask_user_for_mode()

    # Set all devices to the chosen mode
    set_mode_tasks = [dev.set_mode(selected_mode) for dev in devices]
